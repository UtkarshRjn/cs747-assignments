...
class AlgorithmBatched:
    ...
    def give_pull(self):
        # START EDITING HERE
        pull_arms = set()
        count = np.zeros(self.num_arms).astype(int)
        for i in range(self.batch_size):
            arm_index = np.argmax(np.random.beta(self.success + 1, self.failure + 1))
            pull_arms.add(arm_index)
            count[arm_index] += 1
        ...
        assert np.sum(ans) == self.batch_size, "Batch size must be equal to sum of pulls"
        assert len(ans) == len(pull_arms), "Number of arms pulled must be equal to number of unique arms pulled"
        
        return (list(sorted(pull_arms)), ans)
        # END EDITING HERE
    
    def get_reward(self, arm_rewards):
        # START EDITING HERE
        for key, val in arm_rewards.items():
            for r in val:
                if(r == 1):
                    self.success[key] += 1
                else:
