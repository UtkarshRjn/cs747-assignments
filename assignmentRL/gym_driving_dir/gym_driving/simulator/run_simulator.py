...
class Task1():
    ...
    def next_action(self, state):
        ...
        [x, y, v, omega] = state
        angle = math.atan(-y/(350-x))*180/math.pi 
        if(angle < 0 ): angle += 360
        
        angle = (angle + 180)%360
        omega = (omega + 180)%360
        if(abs(angle - omega) > 10):
            action_steer = 2
            action_acc = 2
        else:
            action_steer = 1
...
class Task2():
    ...
    def next_action(self, state, FuckedUp, center):
        """
        Input: The current state
        Output: Action to be taken
        TO BE FILLED
        ...
        if(FuckedUp):
            if(abs(y) >= lo and abs(x) >= lo and not center):
                if(x < 0):
                    if(omega <= e or 360-omega <= e):
                        action_steer = 1
                        action_acc = 4
                    else:
                        action_steer = 2
                    ...
                    if(omega <= 180+e and omega >= 180-e):
                        action_steer = 1
                        action_acc = 4
                    else:
                        action_steer = 2
                        action_acc = 2
            elif(abs(y) >= lo and abs(x) <= lo and not center):
                if(y > 0):
                    if(omega <= 270 + e and omega >= 270 -e):
                        action_steer = 1
                        action_acc = 4
                    else:
                        action_steer = 2
    ...
    def controller_task2(self, config_filepath=None, render_mode=False):
        ...
        for e in range(NUM_EPISODES):
            ...
            for t in range(TIMESTEPS):
                ...
                if t == 0:
                    for i in range(4):
                        if(abs(x - ran_cen_list[i][0]) < 110 and (x/abs(x) == ran_cen_list[i][0]/abs(ran_cen_list[i][0])) and (y/abs(y) == ran_cen_list[i][1]/abs(ran_cen_list[i][1]))):
                            FuckedUp = True
                            break


                action = self.next_action(state,FuckedUp,center)
                state, reward, terminate, reached_road, info_dict = simulator._step(action)
